...
public class SpectatorManager {
    ...
    private final Set<UUID> internalGamemodeChanges;
    ...
    public SpectatorManager(SpectatorPlusPlus plugin) {
        ...
        this.internalGamemodeChanges = new HashSet<>();
    }
    ...
    public void enterSpectator(Player player) {
        ...
        try {
            ...
            beginInternalGamemodeChange(playerId);
            player.setGameMode(GameMode.ADVENTURE);
            endInternalGamemodeChangeLater(playerId, 2L);
    ...
    public void leaveSpectator(Player player) {
        ...
        try {
            ...
            if (originalGameModes.containsKey(playerId)) {
                GameMode originalMode = originalGameModes.get(playerId);
                player.setGameMode(originalMode);
                if (originalMode == GameMode.CREATIVE) {
    ...
    public void spectatePlayer(Player spectator, Player target) {
        ...
        try {
            ...
            beginInternalGamemodeChange(spectator.getUniqueId());
            ...
            plugin.getServer().getScheduler().runTaskLater(plugin, () -> {
                if (spectator.isOnline() && target.isOnline() && isSpectating(spectator)) {
                    try {
                        ...
                        endInternalGamemodeChangeLater(spectator.getUniqueId(), 2L);
                        endInternalGamemodeChangeLater(spectator.getUniqueId(), 2L);
    ...
    public void stopSpectating(Player spectator) {
        ...
        if (spectatingTargets.containsKey(spectatorId)) {
            ...
            beginInternalGamemodeChange(spectatorId);
            spectator.setGameMode(GameMode.ADVENTURE);
            endInternalGamemodeChangeLater(spectatorId, 2L);
    ...
    private void restoreSpectatorProperties(Player player) {
        if (isSpectator(player)) {
            beginInternalGamemodeChange(player.getUniqueId());
            player.setGameMode(GameMode.ADVENTURE);
            endInternalGamemodeChangeLater(player.getUniqueId(), 2L);
            player.setAllowFlight(true);
            player.setFlying(true);
            player.setFlySpeed(0.1f);
            player.setWalkSpeed(0.2f);
    ...
    public void leaveSpectatorWithoutTeleportAndGamemode(Player player) {
        if (!isSpectator(player)) {
            return;
        }
        ...
        visibilityManager.updateAllVisibility();
        visibilityManager.updatePlayerListName(player);
    ...
    private void beginInternalGamemodeChange(UUID playerId) {
        internalGamemodeChanges.add(playerId);
    }
    
    private void endInternalGamemodeChange(UUID playerId) {
        internalGamemodeChanges.remove(playerId);
    }
    
    private void endInternalGamemodeChangeLater(UUID playerId, long delayTicks) {
        plugin.getServer().getScheduler().runTaskLater(plugin, () -> endInternalGamemodeChange(playerId), delayTicks);
    }
    
    public boolean isInternalGamemodeChange(Player player) {
        return internalGamemodeChanges.contains(player.getUniqueId());
    }
    ...
    public boolean isSpectator(Player player) {
        UUID playerId = player.getUniqueId();
        return spectators.contains(playerId) || spectatingTargets.containsKey(playerId);
    }
    ...
    public void ensureSpectatorState(Player player) {
        ...
        if (isSpectating(player)) {
            ...
            boolean hasCompass = false, hasEye = false, hasPaper = false, hasBed = false;
            for (ItemStack item : player.getInventory().getContents()) {
                if (item == null || !item.hasItemMeta()) continue;
                ItemMeta meta = item.getItemMeta();
                if (meta == null || !meta.hasDisplayName()) continue;
                String name = meta.getDisplayName();
                if (name.contains("Spectator Compass")) hasCompass = true;
                else if (name.contains("Toggle Spectator Visibility")) hasEye = true;
                else if (name.contains("Toggle Spectator Chat")) hasPaper = true;
                else if (name.contains("Leave Spectator Mode")) hasBed = true;
            }
            if (!hasCompass || !hasEye || !hasPaper || !hasBed) {
                giveSpectatorItems(player);
            }
        }
